...
% % Xiphoid
Cap2DistModel = load('Cap2DistModel.mat');
...
for n=1:length(FileListT)
   ...
   Time = [Table{:,1}];
   Cap = [Table{:,2}];
   [pred, ~]=predict(Cap2DistModel.mdl,Cap);
   Sensors = [pred Table{:,3}];
...
function [a, b] = getPeaksVals(SensorsTS)
    ...
    % % Find peaks in data
    [Cpks,Clocs,~,~] = findpeaks(SensorsTS.Data(:,1), SensorsTS.Time, 'MinPeakProminence',0.025,'MinPeakDistance',1);
    [Cvals,Cvlocs,~,~] = findpeaks(-SensorsTS.Data(:,1),SensorsTS.Time,'MinPeakProminence',0.025,'MinPeakDistance',1);
