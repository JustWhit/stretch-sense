Folder = 'Y:\Justin\GitRepositories\stretch-sense\Data\MovementData';
...
%     VolF = VolF(2:Length+1);
    ...
    fourierPlot(CapF,TimeF);
%     ShortTimeFourierPlot(CapF,TimeF);
...
% estVT = getVT2(PredTS,Traces{n,5},x0);
% actVT = getVT2(FlowTS,Traces{n,5},x0);
...
function ShortTimeFourierPlot(y,time)
    fs = numel(y)/(time(end)-time(1));
    % % define analysis parameters
    xlen = length(y);                   % length of the signal
...
function fourierPlot(y, time)
   
    Freq = 0.375; %locs(i);
    bandW = 0.45; %2*width(i);
    RR = Freq*60;
    
    fy = FouFilter(y',time(end)-time(1),Freq,bandW,1,0);


    Fs = numel(fy)/(time(end)-time(1));
    % Compute DFT of x
    Y = fft(fy); 
    L = length(fy);
    P2 = abs(Y/L);
...
%     ShortTimeFourierPlot(fy,time)
    
%    	fo = Freq;   %frequency of the sine wave
% 	Ts = 1/Fs; %sampling time interval
% 	t = 0:Ts:numel(time); %sampling period
% 	n = length(t) %number of samples
% 	sy = 2*sin(2*pi*fo*t); %the sine curve

    figure;
    findpeaks(P1,f,'Annotate','extents','MinPeakProminence',0.3);
    figure;
    subplot(3,1,1)
    plot(f,P1) 
    title(['Amplitude Spectrum of X(t): Peak = ' num2str(Freq) 'Hz : estRR = ' num2str(RR) 'bpm'])
    xlabel('f (Hz)')
    ylabel('|P1(f)|')
    subplot(3,1,2)
    plot(time,y)
    title('Raw Capacitance -DC')
    xlabel('Time(s)')
    ylabel('Capacitance(pF)')
    subplot(3,1,3)
    plot(time,fy)
    title('Filtered Capacitance Signal')
    xlabel('Time(s)')
    ylabel('Capacitance(pF)');
