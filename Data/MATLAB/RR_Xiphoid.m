...
C1p = tsC1.Data;
% % Find peaks in data
    [pks,locs,widths,proms] = findpeaks(A1p, A1.Time, 'MinPeakProminence',6);
    [Cpks,Clocs,Cwidths,Cproms] = findpeaks(C1p, C1time, 'MinPeakProminence',6);
    Corr = Clocs(1)-locs(1);

A1 = getsampleusingtime(ts,Ttime(2)-Corr,Ttime(3));
A1p = A1.Data;
length1 = min([numel(A1p) numel(C1p)]);
A1p = A1p(1:length1);
C1p = C1p(1:length1);

A2=getsampleusingtime(ts,Ttime(4)-Corr,Ttime(5));
A2p = A2.Data;
C2p = tsC2.Data;
...
A3 = getsampleusingtime(ts,Ttime(6)-Corr,Ttime(7));
A3p = A3.Data;
C3p = tsC3.Data;
...
A4 = getsampleusingtime(ts2,Ttime2(2)-Corr,Ttime2(3));
A4p = A4.Data;
C4p = tsC4.Data;
...
A5 = getsampleusingtime(ts2,Ttime2(4)-Corr,Ttime2(5));
A5p = A5.Data;
C5p = tsC5.Data;
...
A6 = getsampleusingtime(ts2,Ttime2(6)-Corr,Ttime2(7));
A6p = A6.Data;
C6p = tsC6.Data;
...
%     [Apks4,Alocs4,Awidths4,Aproms4] = findpeaks(A4p, C4time, 'MinPeakProminence',6);
%     [Cpks4,Clocs4,Cwidths4,Cproms4] = findpeaks(C4p, C4time, 'MinPeakProminence',0.25);
%     [Apks5,Alocs5,Awidths5,Aproms5] = findpeaks(A5p, C5time, 'MinPeakProminence',6);
%     [Cpks5,Clocs5,Cwidths5,Cproms5] = findpeaks(C5p, C5time, 'MinPeakProminence',0.25);
%     [Apks6,Alocs6,Awidths6,Aproms6] = findpeaks(A6p, C6time, 'MinPeakProminence',6);
%     [Cpks6,Clocs6,Cwidths6,Cproms6] = findpeaks(C6p, C6time, 'MinPeakProminence',0.25);
% % % Find Valleys in Data
...
%     [Avals4,Avlocs4,Avwidths4,Avproms4] = findpeaks(-A4p,C4time,'MinPeakProminence',6);
%     [Cvals4,Cvlocs4,Cvwidths4,Cvproms4] = findpeaks(-C4p,C4time,'MinPeakProminence',0.25);
%     [Avals5,Avlocs5,Avwidths5,Avproms5] = findpeaks(-A5p,C5time,'MinPeakProminence',6);
%     [Cvals5,Cvlocs5,Cvwidths5,Cvproms5] = findpeaks(-C5p,C5time,'MinPeakProminence',0.25);
%     [Avals6,Avlocs6,Avwidths6,Avproms6] = findpeaks(-A6p,C6time,'MinPeakProminence',6);
%     [Cvals6,Cvlocs6,Cvwidths6,Cvproms6] = findpeaks(-C6p,C6time,'MinPeakProminence',0.25);
%     
%     
%     
%     
